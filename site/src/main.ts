...
if (isDemo) {
  ...
  const oldHeading = byId<HTMLElement>("bench-title");
  const heading = document.createElement("h1");
  heading.id = oldHeading.id;
  heading.textContent = "Compare sample webhook versions";
  oldHeading.replaceWith(heading);
  bench.setAttribute("aria-labelledby", heading.id);
  byId("main").prepend(bench);
  bench.querySelector<HTMLElement>(".fixture-grid")!.before(resultPanel);
  document.title = "Demo — Version Replay";
  const demoDescription = "Try Version Replay with isolated sample webhook fixtures.";
